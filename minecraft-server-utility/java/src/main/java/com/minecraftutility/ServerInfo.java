...
public class ServerInfo {
    ...
    public ServerInfo(boolean online, String host, int port, String jsonData) {
        ...
        JSONObject parsed;
        boolean parsedOnline = online;
        if (jsonData != null && !jsonData.isEmpty()) {
            try {
                parsed = new JSONObject(jsonData);
            } catch (Exception e) {
                // Malformed/unexpected payload from the server - treat as
                // offline rather than letting the constructor throw.
