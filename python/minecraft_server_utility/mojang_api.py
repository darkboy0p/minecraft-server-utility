...
class MojangAPI:
    ...
    def get_name_history(self, uuid: str) -> List[Dict[str, str]]:
        ...
        NOTE: Mojang deprecated and removed the name-history endpoint
        (`/user/profiles/{uuid}/names`). This method now returns an empty
        list instead of raising when the endpoint is unavailable (404/410),
        so callers relying on this data won't crash - they'll just get no
        history back.
        """
        try:
            response = self.session.get(
